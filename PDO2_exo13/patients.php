...
class patients {
    ...
    public function __construct() {
         ...
         */
        try {
            $this->connexion = NEW PDO('mysql:host=localhost;dbname=hospitalE2N;charset=utf8', 'dembasy', '2304');
        }
        /**
...
// Exercice 4
    ...
    public function removePatient() {
        $remove = $this->connexion->prepare('DELETE FROM `patients`'
                . 'WHERE `id` =  :idRemove ');
    ...
    public function getPatientsListByFive($limit, $offset) {
        $result = array();
        $queryResult = $this->connexion->prepare('SELECT `id`,`lastname`, `firstname`, `birthdate`,`phone`, `mail` '
                . 'FROM `patients` '
